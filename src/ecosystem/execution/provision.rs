...
const PRODUCTS_CREATION_SCHEMA_VERSION: u32 = 1;
...
pub async fn ensure(
    ...
    {
        if record["schema_version"] == PRODUCTS_CREATION_SCHEMA_VERSION
            && record["request_id"] == request_id
            && record["initiative_id"] == initiative.id
            && record["state"] == "provisioned"
            && record["product"]["id"].as_str() == Some(id)
        {
        ...
        .unwrap_or(false)
    {
        return Err(AppError::State(
            "signed Las catalogue is unavailable; product provisioning was not dispatched".into(),
        ));
    }
    let request = json!({"schema_version":PRODUCTS_CREATION_SCHEMA_VERSION,"request_id":request_id,"initiative_id":initiative.id,
    ...
    let response = observe::products(
        shared,
        &["create", "--request", path, "--allow-create", "--json"],
    )
    .await?;
