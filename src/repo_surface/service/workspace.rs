...
use crate::repo_surface::state::{CheckEvidence, WorkspaceState};
...
impl RepoService {
    ...
    pub(super) async fn workspace_diff(&self, input: WorkspaceOnly) -> SurfaceResult<Value> {
        let _workspace_lock = self.state.lock_workspace(&input.workspace_id)?;
        let state = self.state.load_workspace(&input.workspace_id)?;
        let repo = self.repo(&state)?;
    ...
    pub(super) async fn workspace_seal(&self, input: WorkspaceOnly) -> SurfaceResult<Value> {
        let _workspace_lock = self.state.lock_workspace(&input.workspace_id)?;
        let mut state = self.state.load_workspace(&input.workspace_id)?;
        let repo = self.repo(&state)?;
        ensure_mutable(&state)?;
    ...
    pub(super) async fn workspace_check(&self, input: WorkspaceCheck) -> SurfaceResult<Value> {
        let _workspace_lock = self.state.lock_workspace(&input.workspace_id)?;
        let mut state = self.state.load_workspace(&input.workspace_id)?;
        let repo = self.repo(&state)?;
        ensure_mutable(&state)?;
