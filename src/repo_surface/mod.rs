...
pub fn tools() -> Value {
    ...
    json!([
      {"name":"workspace_create","description":"Claim the policy-bound canonical main checkout without creating a branch or another checkout.","inputSchema":object(json!({"repo_id":{"type":"string"},"workspace_id":{"type":"string"},"request_id":{"type":"string"}}),json!(["repo_id","workspace_id","request_id"]))},
      ...
      {"name":"branch_publish","description":"Reconcile then publish the exact committed main revision without force; release still requires independent acceptance.","inputSchema":object(json!({"workspace_id":{"type":"string"},"request_id":{"type":"string"}}),json!(["workspace_id","request_id"]))},
